  ...
  function render(){
    ...
    for(const item of filtered()){
      ...
      if(view.value==='applications'){
        ...
        const save=document.createElement('button');save.type='submit';save.textContent='Save review';form.append(label,noteLabel,save);form.addEventListener('submit',async event=>{event.preventDefault();await mutate(item,'status',{status:select.value,note:note.value,expectedVersion:item.version??0},save);});details.append(form);
        if(['under_review','waitlisted','offered'].includes(item.status)){const offer=document.createElement('button');offer.type='button';offer.textContent=item.status==='offered'?'Reissue offer link':'Issue offer';offer.addEventListener('click',()=>mutate(item,'offer',{expectedVersion:item.version??0},offer));details.append(offer);}

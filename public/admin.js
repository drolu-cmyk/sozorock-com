  ...
  function render(){
    ...
    for(const item of filtered()){
      ...
      if(view.value==='applications'){
        ...
        if(['under_review','waitlisted'].includes(item.status)){const offer=document.createElement('button');offer.type='button';offer.textContent='Issue offer';offer.addEventListener('click',()=>mutate(item,'offer',{expectedVersion:item.version??0},offer));details.append(offer);}

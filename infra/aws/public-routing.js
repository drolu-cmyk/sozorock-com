...
function handler(event) {
 ...
 if(/^\/releases\/[a-f0-9]{40}\/(corporate\.css|corporate\.js|cbcap-preview\.js|school\.css|school-nav\.js|applications\.js|offers\.js|admin\.js|contact\.js|favicon\.svg|favicon-48\.png|apple-touch-icon\.png)$/.test(uri))return r;
 if(routes.indexOf(uri)!==-1)r.uri=(release?'/releases/'+release:'')+(uri==='/'?'/index.html':uri+'/index.html');
 else if(/^\/(assets|media)\/[a-zA-Z0-9_./-]+$/.test(uri)&&uri.indexOf('..')===-1||/^\/(corporate\.css|corporate\.js|cbcap-preview\.js|school\.css|school-nav\.js|applications\.js|offers\.js|admin\.js|contact\.js|favicon\.svg|favicon-48\.png|apple-touch-icon\.png|menu\.svg|robots\.txt|sitemap\.xml|social-card\.(png|svg)|404\.html)$/.test(uri)){

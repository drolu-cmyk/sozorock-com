...
API throttling bounds incoming request rates. Lambda uses shared regional concurrency by default because the first deployment could not reserve five executions without violating the account’s unreserved-capacity minimum. This does not guarantee capacity or impose a per-function concurrency cap. Set `ReserveFunctionCapacity=true` only after checking available account capacity; it reserves and caps the function at five concurrent executions. CORS/honeypots are not bot authentication. Monitor initial intake before enabling broadly; persistent abuse requires a demonstrated additional control. No email notification, application decision workflow, payment collection, or self-service deletion endpoint is claimed. Admin recovery is operator-assisted. Preserve a separate authorized recovery operator before onboarding the sole administrator.

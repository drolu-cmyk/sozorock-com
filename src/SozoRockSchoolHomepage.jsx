...
const programs = [{
  ...
  feedback: 'Before an external action gives a person a chance to verify the result before it affects someone. The right checkpoint also depends on the impact of the task.',
  ...
  description: 'Follow an identity from arrival to departure. Explore how the right permissions protect the organisation while letting people do their work.',
...
export function SozoRockSchoolHomepage() {
  ...
  return <><a className="skip" href="#courses">Skip to programs</a><section className="hero"><img className="hero-image" src="/media/ripple-04-impact.png" alt="A smooth stone surrounded by widening rings on dark green water" /><div className="hero-shade" /><header><a href="#" className="brand" aria-label="SozoRock School home">SozoRock<span>School</span></a><nav aria-label="Main navigation"><a href="#courses">Courses</a><a href="#experience">Experience</a><a href="#outcomes">Outcomes</a></nav><a className="button light header-cta" href="#courses">Find your program <ArrowUpRight size={18} /></a></header><div className="hero-copy"><h1>Learn it.<br />Put it to work.</h1><p>Hands-on learning in AI systems,<br className="desktop-break" /> cybersecurity and AI governance.</p><div className="hero-actions"><a className="button light" href="#courses">Explore the programs <ArrowUpRight size={20} /></a><a className="text-link" href="#experience">How learning works <ArrowRight size={18} /></a></div></div><button className="stone-secret" aria-label="Make a ripple" onClick={() => setSecret(v => !v)}><span className={secret ? 'ripple active' : 'ripple'} /><span className="secret-label">{secret ? 'Small decisions. Lasting impact.' : 'Make a ripple'}</span></button><div className="hero-foot"><span>Four disciplines. Work that matters.</span><a href="#courses" aria-label="Discover the programs"><ChevronDown /></a></div></section>
 <section id="courses" className="section explorer"><div className="section-heading"><h2>Where do you want<br />to make an impact?</h2><p>Start with the kind of problem you want to solve.<br />Try a decision. Find a direction.</p></div><div className="explorer-grid"><div className="program-picker" aria-label="Choose a program">{programs.map(item => <button key={item.id} className={active === item.id ? 'active' : ''} aria-pressed={active === item.id} onClick={() => {
            setActive(item.id);
            setFocusRequest(0);
